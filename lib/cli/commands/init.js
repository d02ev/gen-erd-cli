...
async function promptForDbConfig(dbType) {
  const connectionType = await selectConnectionType(dbType);
  showSuccessMessage('Connection type selected', connectionType);

  let config = { connectionType };

  if (connectionType === CONNECTION_TYPE_CONNECTION_STRING) {
    const connectionString = await getConnectionString(dbType);
    showSuccessMessage('Connection string received');
    config.connectionString = connectionString;
  } else if (connectionType === CONNECTION_TYPE_WINDOWS_AUTH) {
    const fields = await getWindowsAuthFields();
    showSuccessMessage('Windows Auth fields received');
    config = { ...config, ...fields };
  } else if (connectionType === CONNECTION_TYPE_AZURE_AD) {
    const fields = await getAzureAdFields(dbType);
    showSuccessMessage('Azure AD fields received');
    config = { ...config, ...fields };
  } else if (connectionType === CONNECTION_TYPE_AWS_IAM) {
    const fields = await getAwsIamFields();
    showSuccessMessage('AWS IAM fields received');
    config = { ...config, ...fields };
  } else {
