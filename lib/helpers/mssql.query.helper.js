 ...
 * MSSQL Query Helper
 * 
 * Note: Array parameters in queries are handled by MSSQLProvider.query() method.
 * The provider expands array parameters into individual named parameters before execution.
 * Do not use this query helper directly without going through the provider.
 */
export default class MSSQLQueryHelper {
  ...
  requestFetchFkMetadataQuery = () => {
    return `
      ...
      AND OBJECT_NAME(fk.parent_object_id) IN (@param1);
    `;
  };
}
