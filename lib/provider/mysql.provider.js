 ...
 * 
 * Note: Array parameters in queries are handled by the query() method.
 * The provider expands array parameters into individual placeholders.
 * Do not use this provider's query method without understanding this behavior.
 */
export default class MySQLProvider {
  ...
  async query(sql, params) {
    try {
      ...
      let paramIndex = 0;
      const expandedSql = sql.replace(/\?/g, () => {
      ...
      const [rows] = await this.connection.execute(expandedSql, expandedParams);
      return { rows };
    } catch (err) {
